...
pub(crate) mod tests {
    ...
    use crate::t_ed25519::{ExpandedKeyPair, Signature};
    ...
    fn test_generate_pubkey_dalek() {
        ...
        for _ in 0..4096 {
            ...
            let keypuzzle_keypair = ExpandedKeyPair::create_from_private_key(privkey);
            ...
            let keypuzzle_pub_serialized = &*keypuzzle_keypair.public_key.to_bytes(true);
            let dalek_pub_serialized = dalek_pub.to_bytes();

            assert_eq!(keypuzzle_pub_serialized, dalek_pub_serialized);
        }
    }
    ...
    fn test_verify_dalek_signatures() {
        ...
        for msg_len in 0..msg.len() {
            ...
            for _ in 0..20 {
                ...
                let keypuzzle_sig = Signature {
                    R: Point::from_bytes(&dalek_sig.as_ref()[..32]).unwrap(),
                    s: Scalar::from_bytes(&dalek_sig.as_ref()[32..]).unwrap(),
                };
                let keypuzzle_pubkey = Point::from_bytes(&dalek_pub.to_bytes()).unwrap();
                keypuzzle_sig.verify(msg, &keypuzzle_pubkey).unwrap();
            }
        }
    }
}
